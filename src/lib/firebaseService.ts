...
// Define UserWriteData interface for Firestore operations
...
function safeCreateDate(timestamp: any): Date | undefined {
  ...
  if (timestamp && typeof timestamp.seconds === "number" && typeof timestamp.nanoseconds === "number") {
    try {
      ...
      console.warn(
        "[firebaseService safeCreateDate] Failed to convert object to Timestamp, then to Date:",
        e,
        "Timestamp data:",
        timestamp,
      )
...
const mapDocToUser = (docSnap: QueryDocumentSnapshot<DocumentData> | DocumentSnapshot<DocumentData>): User => {
  const data = docSnap.data()
  if (!data) {
    console.error(
      `[firebaseService mapDocToUser] Document data is null/undefined for doc ID: ${docSnap.id}. Returning minimal error user.`,
    )
    return {
      id: docSnap.id,
  ...
  const user: User = {
    ...
    hourlyRate:
      data.role === "developer" ? (typeof data.hourlyRate === "number" ? data.hourlyRate : undefined) : undefined,
    portfolioUrls:
      data.role === "developer"
        ? Array.isArray(data.portfolioUrls)
          ? data.portfolioUrls.filter(Boolean)
          : []
        : undefined,
...
export async function addUser(
  ...
  const documentToWrite: WithFieldValue<UserWriteData> = {
    ...
    bio:
      userData.role === "developer"
        ? "Enthusiastic developer ready to craft some code!"
        : "Client looking for talented developers.",
  }
  ...
  if (userData.role === "developer") {
    if (
      ...
      userData.skills.some((s) => s && s.trim() !== "")
    ) {
    ...
    if (
      ...
      Array.isArray(userData.portfolioUrls) &&
      userData.portfolioUrls.length > 0 &&
      userData.portfolioUrls.some(
        (url) => url && url.trim() !== "" && (url.startsWith("http://") || url.startsWith("https://")),
      )
    ) {
      documentToWrite.portfolioUrls = userData.portfolioUrls.filter(
        (url) => url && url.trim() !== "" && (url.startsWith("http://") || url.startsWith("https://")),
      )
    }
    ...
    if (
      ...
      userData.resumeFileUrl.trim() !== "" &&
      (userData.resumeFileUrl.startsWith("http://") || userData.resumeFileUrl.startsWith("https://"))
    ) {
    ...
    if (
      ...
      !isNaN(Number(userData.hourlyRate)) &&
      Number(userData.hourlyRate) >= 0
    ) {
      documentToWrite.hourlyRate = Number(userData.hourlyRate)
    }
  }
  console.log(
    "[firebaseService addUser] documentToWrite before setDoc:",
    JSON.stringify(
      documentToWrite,
      (k, v) => {
  ...
  try {
    ...
    if (documentToWrite.name && documentToWrite.email && documentToWrite.role) {
      ...
      console.warn(
        `[firebaseService addUser] Could not send welcome email due to missing name, email, or role for user ${userId}`,
      )
    }
    ...
    if (
      ...
      console.error(
        `[firebaseService addUser] Firestore 'invalid-argument' error. This often means an 'undefined' value was passed for a field. Check document:`,
        documentToWrite,
      )
    }
...
export async function getUserByEmail(email: string): Promise<User | null> {
  if (!db) {
    console.error(
      `[firebaseService getUserByEmail] Firestore is not initialized attempting to fetch user by email: ${email}`,
    )
    throw new Error("Firestore is not initialized. Check Firebase configuration.")
  }
  ...
  try {
    const usersQuery = query(collection(db, USERS_COLLECTION), where("email", "==", lowercasedEmail), limit(1))
    const querySnapshot = await getDocs(usersQuery)
    ...
    console.error(
      `[firebaseService getUserByEmail] Error fetching user by email ${lowercasedEmail} from Firestore: `,
      error,
    )
    if (error instanceof Error) {
...
export async function updateUser(
  userId: string,
  data: Partial<Omit<User, "id" | "createdAt" | "email" | "role" | "referralCode" | "currentPlan" | "planPrice">>,
): Promise<void> {
  if (!db) {
  ...
  try {
    ...
    if (data.name !== undefined) {
      updateData.name = data.name && data.name.trim() ? data.name.trim() : deleteField()
    }
    // Bio
    ...
    if (data.avatarUrl !== undefined) {
      const trimmedAvatarUrl = data.avatarUrl ? data.avatarUrl.trim() : ""
      if (trimmedAvatarUrl && (trimmedAvatarUrl.startsWith("http://") || trimmedAvatarUrl.startsWith("https://"))) {
        ...
        // Invalid URL or non-empty, non-URL string likely intended for deletion
    ...
    if (effectiveRole === "developer") {
      if (data.skills !== undefined) {
        updateData.skills =
          Array.isArray(data.skills) && data.skills.length > 0 && data.skills.some((s) => s && s.trim() !== "")
            ? data.skills.filter((s) => s && s.trim() !== "")
            : deleteField()
      }
      if (data.experienceLevel !== undefined) {
        updateData.experienceLevel =
          data.experienceLevel && data.experienceLevel.trim() !== "" ? data.experienceLevel : deleteField()
      }
      if (data.hourlyRate !== undefined) {
        // Check if hourlyRate key exists in data
        updateData.hourlyRate =
          typeof data.hourlyRate === "number" && data.hourlyRate >= 0 ? data.hourlyRate : deleteField()
      }
      if (data.portfolioUrls !== undefined) {
        updateData.portfolioUrls =
          Array.isArray(data.portfolioUrls) &&
          data.portfolioUrls.length > 0 &&
          data.portfolioUrls.some(
            (url) => url && url.trim() !== "" && (url.startsWith("http://") || url.startsWith("https://")),
          )
            ? data.portfolioUrls.filter(
                (url) => url && url.trim() !== "" && (url.startsWith("http://") || url.startsWith("https://")),
              )
            : deleteField()
      }
      if (data.resumeFileUrl !== undefined) {
        updateData.resumeFileUrl =
          data.resumeFileUrl &&
          data.resumeFileUrl.trim() &&
          (data.resumeFileUrl.startsWith("http://") || data.resumeFileUrl.startsWith("https://"))
            ? data.resumeFileUrl.trim()
            : deleteField()
      }
      if (data.resumeFileName !== undefined) {
        // resumeFileName can be an empty string if resumeFileUrl is also empty
        updateData.resumeFileName =
          data.resumeFileUrl && data.resumeFileUrl.trim()
            ? data.resumeFileName && data.resumeFileName.trim()
              ? data.resumeFileName.trim()
              : deleteField()
            : deleteField()
      }
      if (data.pastProjects !== undefined) {
        updateData.pastProjects =
          data.pastProjects && data.pastProjects.trim() ? data.pastProjects.trim() : deleteField()
      }
    } else {
      // If role is not developer, ensure these fields are removed if they exist
...
export async function addProject(
  ...
  try {
    ...
    if (!fetchedProject) {
      console.error(
        `[firebaseService addProject] Project ${projectDocRef.id} was supposedly added but could not be retrieved.`,
      )
    ...
    if (clientEmail && clientName) {
      try {
        const projectEmailHtml = await getClientProjectPostedEmailTemplate(
          clientName,
          fetchedProject.name,
          fetchedProject.id,
        )
        ...
        console.error(
          `[firebaseService addProject] Failed to send project confirmation email for project ${fetchedProject.id}. Email error:`,
          emailError instanceof Error ? emailError.message : emailError,
        )
      }
    }
  ...
  try {
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      if (!data.name || !data.clientId || !data.createdAt || !(data.createdAt instanceof Timestamp)) {
        console.warn(
          `[firebaseService getProjectsByClientId] Project ${docSnap.id} has missing essential fields (name, clientId, or createdAt is not Timestamp), skipping.`,
        )
        return
      }
    ...
    if (error instanceof Error) {
      if (error.message.includes("The query requires an index.")) {
        throw new Error(
          `Firestore query for client projects failed. It requires a composite index. Please check the Firebase console for a link to create it. Error: ${error.message}`,
        )
      }
  ...
  try {
    ...
    if (projectDocSnap.exists()) {
      ...
      if (
        ...
        console.warn(
          `[firebaseService getProjectById] Project ${projectId} has missing or invalid essential fields. Data:`,
          data,
        )
        return null
      }
...
export async function getAllProjects(): Promise<Project[]> {
  ...
  try {
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      if (!data.name || !data.clientId || !data.createdAt || !(data.createdAt instanceof Timestamp)) {
        console.warn(
          `[firebaseService getAllProjects] Project ${docSnap.id} has missing essential fields (name, clientId, or createdAt is not Timestamp), skipping.`,
        )
        return
      }
      let statusValue = data.status || "Unknown"
...
export async function getReferredClients(currentUserReferralCode: string): Promise<User[]> {
  if (!db) {
    console.error(
      `[firebaseService getReferredClients] Firestore is not initialized for referral code: ${currentUserReferralCode}`,
    )
    throw new Error("Firestore is not initialized. Check Firebase configuration.")
  }
  ...
  try {
    ...
    console.error(
      `[firebaseService getReferredClients] Error fetching referred clients for code ${currentUserReferralCode}: `,
      error,
    )
    if (error instanceof Error) {
      if (error.message.includes("The query requires an index.")) {
        throw new Error(
          `Firestore query for referred clients failed. It requires a composite index. Please check the Firebase console for a link to create it. Error: ${error.message}`,
        )
      }
...
export async function updateUserAccountStatus(
  ...
  try {
    ...
    try {
      ...
      console.error(
        `[firebaseService updateUserAccountStatus] Failed to send account status notification email to ${userEmail} for status ${newStatus}. Email error:`,
        emailError instanceof Error ? emailError.message : emailError,
      )
    }
  } catch (error) {
    console.error(
      `[firebaseService updateUserAccountStatus] Error updating account status for user ${userId} to ${newStatus}:`,
      error,
    )
    if (error instanceof Error) {
...
export async function addProjectApplication(
  applicationData: Omit<
    ProjectApplication,
    "id" | "appliedAt" | "status" | "clientNotifiedOfNewApplication" | "developerNotifiedOfStatus"
  >,
  ...
  try {
    const existingApplications = await getApplicationsByDeveloperForProject(
      applicationData.developerId,
    ...
    if (project && project.clientId) {
      ...
      if (client && client.email && client.name) {
        try {
          const appEmailHtml = await getNewProjectApplicationEmailToClient(
            client.name,
            applicationData.developerName,
          ...
          console.error(
            `[firebaseService addProjectApplication] Failed to send application notification email to client ${client.email} for app ${appDocRef.id}. Email error:`,
            emailError instanceof Error ? emailError.message : emailError,
          )
        }
      }
    ...
    if (!newAppSnap.exists()) {
      console.error(
        `[firebaseService addProjectApplication] Application ${appDocRef.id} was supposedly added but could not be retrieved.`,
      )
    ...
    if (!(data.appliedAt instanceof Timestamp)) {
      console.warn(
        `[firebaseService addProjectApplication] Expected data.appliedAt to be a Firestore Timestamp for app ${newAppSnap.id} after creation. Got:`,
        typeof data.appliedAt,
        ". Using current server time as fallback.",
      )
...
export async function getApplicationsByDeveloperForProject(
  developerId: string,
  projectId: string,
): Promise<ProjectApplication[]> {
  if (!db) {
    console.error(
      `[firebaseService getApplicationsByDeveloperForProject] Firestore is not initialized for dev ${developerId}, project ${projectId}.`,
    )
  ...
  try {
    ...
    return querySnapshot.docs.map((docSnap) => {
      ...
      if (!(data.appliedAt instanceof Timestamp)) {
        console.warn(
          `[firebaseService getApplicationsByDeveloperForProject] Application ${docSnap.id} 'appliedAt' is not a Firestore Timestamp. Using current date as fallback.`,
        )
        data.appliedAt = Timestamp.now()
      }
      return {
    ...
    console.error(
      `[firebaseService getApplicationsByDeveloperForProject] Error fetching applications for dev ${developerId} on project ${projectId}:`,
      error,
    )
...
export async function getApplicationsByProjectId(projectId: string): Promise<ProjectApplication[]> {
  ...
  try {
    ...
    return querySnapshot.docs.map((docSnap) => {
      ...
      if (!(data.appliedAt instanceof Timestamp)) {
        console.warn(
          `[firebaseService getApplicationsByProjectId] Application ${docSnap.id} 'appliedAt' is not a Firestore Timestamp. Using current date as fallback.`,
        )
        data.appliedAt = Timestamp.now()
      }
      return {
    ...
    console.error(
      `[firebaseService getApplicationsByProjectId] Error fetching applications for project ${projectId}:`,
      error,
    )
    if (error instanceof Error) {
      if (error.message.includes("The query requires an index.")) {
        throw new Error(
          `Firestore query for project applications failed. It requires a composite index (projectId asc, appliedAt desc). Please check the Firebase console for a link to create it. Error: ${error.message}`,
        )
      }
      throw error
    }
...
export async function updateProjectApplicationStatus(
  ...
  if (!db) {
    console.error(
      `[firebaseService updateProjectApplicationStatus] Firestore is not initialized for application ${applicationId}.`,
    )
    throw new Error("Firestore is not initialized.")
  }
  ...
  try {
    ...
    if (
      newStatus === "rejected" &&
      appData.developerEmail &&
      appData.developerName &&
      appData.projectName &&
      appData.projectId
    ) {
      try {
        const emailHtml = await getApplicationRejectedEmailToDeveloper(
          appData.developerName,
          appData.projectName,
          appData.projectId,
        )
    ...
    console.error(
      `[firebaseService updateProjectApplicationStatus] Error updating application ${applicationId} to ${newStatus}:`,
      error,
    )
    if (error instanceof Error) throw error
...
export async function assignDeveloperToProject(
  ...
  try {
    ...
    try {
      ...
      console.error(
        `[firebaseService assignDeveloperToProject] Failed to send acceptance email to ${developerEmail} or update notification status for app ${applicationId}. Email error:`,
        emailError instanceof Error ? emailError.message : emailError,
      )
    }
    ...
    console.error(
      `[firebaseService assignDeveloperToProject] Error assigning developer ${developerId} to project ${projectId}:`,
      error,
    )
    if (error instanceof Error) throw error
...
export async function rejectOtherPendingApplications(
  ...
  try {
    ...
    querySnapshot.forEach((docSnap) => {
      if (docSnap.id !== acceptedApplicationId) {
        ...
        addAdminActivityLog({
          ...
          console.error("[firebaseService rejectOtherPendingApplications] Error logging auto-rejection:", logError),
        )
      }
    })
    ...
    console.error(
      `[firebaseService rejectOtherPendingApplications] Error rejecting other applications for project ${projectId}:`,
      error,
    )
    if (error instanceof Error) {
      throw error
